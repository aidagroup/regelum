...
class XWriter(rg.Node):
    ...
    class Inputs(rg.NodeInputs):
        x: int = rg.src("XWriter.x")
        y: int = rg.src("YWriter.y")

    class State(rg.NodeState):
        x: int = rg.var(init=0, domain=(0, 1))
...
class YWriter(rg.Node):
    ...
    class State(rg.NodeState):
        y: int = rg.var(init=0, domain=(0, 1))
...
def build_system(
    ...
    g1 = (rg.V("XWriter.x") == 0) & (rg.V("YWriter.y") == 0)
    g2 = (rg.V("XWriter.x") == 0) & (rg.V("YWriter.y") == 1)
    ...
    return rg.PhasedReactiveSystem(
        phases=[
            rg.Phase(
                ...
                nodes=(x_writer,),
                transitions=(
                    rg.If(g1, "psi2", name="g1"),
                    rg.If(~g1, rg.terminate, name="exit-psi1"),
                ),
                is_initial=True,
            ),
            rg.Phase(
                ...
                nodes=(y_writer,),
                transitions=(
                    rg.If(g2, "psi1", name="g2"),
                    rg.If(~g2, rg.terminate, name="exit-psi2"),
                ),
            ),
        ],
    )
