...
class Accumulator(rg.Node):
    ...
    class Inputs(rg.NodeInputs):
        counter: int = rg.src("Accumulator.counter")

    class State(rg.NodeState):
        counter: int = rg.var(init=0)
...
def build_system(
    ...
    early_exit = rg.V("Accumulator.counter") >= threshold
    accumulator = Accumulator(step_size=step_size)

    phases: list[rg.Phase] = []
    for index in range(max_steps):
        ...
        phases.append(
            rg.Phase(
                ...
                nodes=(accumulator,),
                transitions=(
                    rg.If(early_exit, rg.terminate, name="early-exit"),
                    rg.If(~early_exit, next_target, name="continue"),
                ),
                is_initial=(index == 0),
            )
        )

    return rg.PhasedReactiveSystem(phases=phases)
