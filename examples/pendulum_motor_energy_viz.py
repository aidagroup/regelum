"""Example of pendulum with motor control using energy-based controller."""

import numpy as np
from regelum.node.classic_control.envs.continuous import (
    PendulumWithMotor,
)
...
class RewardPendulum(RewardTracker):
    ...
    def objective_function(self, x: np.ndarray) -> float:
        angle_error = x[0]
        return float(4 * angle_error**2 + x[1] ** 2 + 0.1 * x[2] ** 2)


# Create pendulum system with motor
pendulum = PendulumWithMotor(
    control_signal_name="energy_swing_up_1.action",
    initial_state=np.array([np.pi, 0.0, 0.0]),  # Start from downward position
    motor_params={
        "mass": 0.1,
        "radius": 0.04,
        "time_const": 0.05,
...
# Create energy-based controller with motor compensation
controller = EnergyBasedSwingUpController(
    ...
    pendulum_params={
        ...
        "motor_inertia": pendulum.motor_moment_inertia(),
    },
