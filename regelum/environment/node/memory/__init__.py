...
def create_memory_chain(
    ...
    cells.append(MemoryCell(target_node, paths_to_remember, step_size))

    # Each subsequent cell remembers previous cell's state
    for _ in range(n_cells - 1):
        prev_cell = cells[-1]
        remembered_paths = [
            f"memory/previous/{path.split('/')[-1]}" for path in paths_to_remember
        ]
        cells.append(MemoryCell(prev_cell, remembered_paths, step_size))
