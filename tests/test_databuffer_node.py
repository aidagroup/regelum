...
def test_buffer_initialization(data_buffer):
    ...
    # Check buffer shapes
    assert data_buffer.state["buffer/pos"].shape == (5, 2)
    assert data_buffer.state["buffer/vel"].shape == (5, 2)
...
def test_buffer_filling(graph, data_buffer):
    ...
    assert not data_buffer.is_buffer_full
    assert np.allclose(data_buffer.state["buffer/pos"].data[0], np.array([0.1, 0.1]))
    assert np.allclose(data_buffer.state["buffer/vel"].data[0], np.array([1.0, 1.0]))
    ...
    assert np.allclose(data_buffer.state["buffer/pos"].data[-1], np.array([0.5, 0.5]))
