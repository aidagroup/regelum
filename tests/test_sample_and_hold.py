...
class TerminateSignal(Node):
    state = State("terminate_pendulum_state", (1,), np.array([False]))
    ...
    def compute_state_dynamics(self):
        ...
        return {"terminate_pendulum_state": np.array([should_terminate])}


@pytest.fixture
def pendulum_system():
    pd_controller = PendulumPDController(20, 20, step_size=0.01)
    pendulum = Pendulum(is_root=True, is_continuous=True, step_size=0.001)
    ...
    graph = Graph(
        ...
        states_to_log=[
            "pendulum_state",
            "pendulum_pd_control",
            "terminate_pendulum_state",
        ],
    )

    # Apply both factories
    reset_factory = ResetFactory()
    sample_and_hold = SampleAndHoldFactory()

    pd_controller.with_transistor(
        pd_controller.default_transistor_configuration["transistor"]
        .with_modifier(reset_factory)
        .with_modifier(sample_and_hold)
    )
